...
import org.cibertec.edu.pe.repository.IDetalleCompraRepository; 
...
public class ProductoController {
    ...
    @Autowired
    private ICompraRepository compraRepository; // Inyecta el repositorio de Compra
    
    @Autowired
    private IDetalleCompraRepository detalleCompraRepository; // Inyecta el repositorio de DetalleCompra
    ...
    public String agregar(Model model, HttpSession session, @PathVariable(name = "idProducto") int idProducto) {
        ...
        if (producto != null) {
            ...
            detalle.setCantidad(1); // Por defecto, se agrega una unidad
    ...
    public String pagar(Model model, HttpSession session) {
        ...
        if (carrito != null) {
            ...
            for (Detalle detalle : carrito) {
                ...
                detalleCompra.setProducto(detalle.getProducto());
                detalleCompra.setCantidad(detalle.getCantidad());
                detalleCompra.setSubtotal(detalle.getSubtotal());
